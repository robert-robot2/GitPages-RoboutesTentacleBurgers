...
namespace SpectralXGLX.SpectralXComponent
{
    public partial class SpectralXEngine
    {
        public void InitScene6()
        {
            ...
            Weather.Init(Scene6, MeshLibrary, new Dictionary<WeatherParticleType, ParticleVolume>
    {
        ...
        "/iAssets/IceTile010.png",
            ...
            // ── Signpost ─────────────────────────────────────────
            AddSignpost(Scene6, "Scene6_SignpostToForest1",
      GetTriggerPos(SceneID.BWPScene3, SceneID.BWPScene1));

            AddSignpost(Scene6, "Scene6_SignpostToForest3",
                GetTriggerPos(SceneID.BWPScene3, SceneID.BWPScene4));
            ...
            SpectralDynManager.SpawnAll(Scene6, MeshLibrary, spawnRadius: 60f, seed: 42);
          //  SpectralBreakManager.SpawnAll(Scene6, MeshLibrary, spawnRadius: 12f, seed: 99);
            ...
            WaveSystem.RegisterSceneWaves(SceneID.BWPScene3, new WaveDefinition[]
      {
    // Wave 1
    new WaveDefinition { Skeletons = new[]{3}, PsychoSkeletons = new[]{1}, ZombiePsycho = new[]{0}, Goatman = new[]{0}, SkeletonWar = new[]{0}, ScavBoss = new[]{0}, SkeletonBoss = new[]{0}, OnWaveStart = () => Console.WriteLine("[Scene6] Wave 1 started!") },
    // Wave 2
    new WaveDefinition { Skeletons = new[]{3}, PsychoSkeletons = new[]{2}, ZombiePsycho = new[]{0}, Goatman = new[]{0}, SkeletonWar = new[]{0}, ScavBoss = new[]{0}, SkeletonBoss = new[]{0} },
    // Wave 3
    new WaveDefinition { Skeletons = new[]{4}, PsychoSkeletons = new[]{2}, ZombiePsycho = new[]{1}, Goatman = new[]{0}, SkeletonWar = new[]{0}, ScavBoss = new[]{0}, SkeletonBoss = new[]{0} },
    // Wave 4
    ...
    // Wave 6
    new WaveDefinition { Skeletons = new[]{3}, PsychoSkeletons = new[]{3}, ZombiePsycho = new[]{2}, Goatman = new[]{0}, SkeletonWar = new[]{0}, ScavBoss = new[]{0}, SkeletonBoss = new[]{0} },
    // Wave 7
    new WaveDefinition { Skeletons = new[]{3}, PsychoSkeletons = new[]{2}, ZombiePsycho = new[]{3}, Goatman = new[]{0}, SkeletonWar = new[]{0}, ScavBoss = new[]{0}, SkeletonBoss = new[]{0} },
    // Wave 8
    new WaveDefinition { Skeletons = new[]{2}, PsychoSkeletons = new[]{2}, ZombiePsycho = new[]{3}, Goatman = new[]{0}, SkeletonWar = new[]{0}, ScavBoss = new[]{0}, SkeletonBoss = new[]{0} },
    // Wave 9
    new WaveDefinition { Skeletons = new[]{2}, PsychoSkeletons = new[]{1}, ZombiePsycho = new[]{3}, Goatman = new[]{0}, SkeletonWar = new[]{0}, ScavBoss = new[]{0}, SkeletonBoss = new[]{0} },
    // Wave 10
    new WaveDefinition { Skeletons = new[]{1}, PsychoSkeletons = new[]{1}, ZombiePsycho = new[]{4}, Goatman = new[]{0}, SkeletonWar = new[]{0}, ScavBoss = new[]{0}, SkeletonBoss = new[]{0} },
      });

            WaveSystem.SetScene(SceneID.BWPScene3);
            WaveSystem.LoadWave(1);

            for (int i = 0; i < 3; i++)
            {
                var (cx, cy) = RandomScatterPoint(0f, 0f, 12f);
                Cows.Add(SpawnCow(Scene6, cx, cy, 1.0f));
            }
            for (int i = 0; i < 3; i++)
            {
                var (catx, caty) = RandomScatterPoint(0f, 0f, 12f);
                Cats.Add(SpawnCat(Scene6, catx, caty, 1.0f));
            }
            for (int i = 0; i < 3; i++)
            {
                var (tsx, tsy) = RandomScatterPoint(0f, 0f, 12f);
                TownSluts.Add(SpawnTownSlut(Scene6, tsx, tsy, 1.0f));
            }

            Console.WriteLine("[SpectralXEngine] Skeleton spawned from menu");
            Console.WriteLine("[InitScene6] Scene6 ready");
        }
