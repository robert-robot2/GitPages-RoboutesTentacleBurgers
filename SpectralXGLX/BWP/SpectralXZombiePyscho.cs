...
{
    /// <summary>
    /// WebGL2 rebuild of ZombiePyscho.
    /// Aggressive zombie variant — chases the active character with
    /// slow, relentless movement and melee attacks. Uses dedicated
    /// zombie sprite sheets and a Psychotic Rage resource.
    /// </summary>
    public class SpectralXZombiePsycho : ISpectralEnemy
    {
        // ── Mesh Reference ────────────────────────────────────────
        ...
        // ── Core Stats ────────────────────────────────────────────
        public int EnemyHitPoints { get; set; } = 6;
        public int EnemyMaxHP { get; set; } = 6;
        ...
        public int EnemyCelerity { get; set; } = 1;
        ...
        // ── Hit / Death Phase ─────────────────────────────────────
        private enum ZombiePhase { Alive, HitEffect, Dead }
        private ZombiePhase _phase = ZombiePhase.Alive;
        ...
        private ZombieAnimationState _currentAnimation = ZombieAnimationState.Idle;
        ...
        private const float AttackCooldownDuration = 0.8f;

        // ── Sprite Sheet Paths ────────────────────────────────────
        public static readonly Dictionary<ZombieAnimationState, string> SpritePaths = new()
        {
            { ZombieAnimationState.Idle,      "/iAssets/ZombieIdleCell01.png"        },
            { ZombieAnimationState.WalkLeft,  "/iAssets/ZombieLeftWalkCell01.png"    },
            { ZombieAnimationState.WalkRight, "/iAssets/ZombieRightWalkCell01.png"   },
            { ZombieAnimationState.Attack,    "/iAssets/ZombieAttackCell01.png"      },
        };
        ...
        // ── Frame Counts ──────────────────────────────────────────
        public static readonly Dictionary<ZombieAnimationState, int> FrameCounts = new()
        {
            { ZombieAnimationState.Idle,      10 },
            { ZombieAnimationState.WalkLeft,  23 },
            { ZombieAnimationState.WalkRight, 23 },
            { ZombieAnimationState.Attack,    12 },
        };

        // ── Animation Speeds (seconds per frame) ──────────────────
        public static readonly Dictionary<ZombieAnimationState, float> AnimSpeeds = new()
        {
            { ZombieAnimationState.Idle,      0.12f },
            { ZombieAnimationState.WalkLeft,  0.05f },
            { ZombieAnimationState.WalkRight, 0.05f },
            { ZombieAnimationState.Attack,    0.05f },
        };
        ...
        public static readonly Dictionary<ZombieAnimationState, float> SheetWidths = new()
        {
            { ZombieAnimationState.Idle,      10 * FrameW }, // 840px
            { ZombieAnimationState.WalkLeft,  23 * FrameW }, // 1932px
            { ZombieAnimationState.WalkRight, 23 * FrameW }, // 1932px
            { ZombieAnimationState.Attack,    12 * FrameW }, // 1008px
        };
        ...
        // ── Constructor ───────────────────────────────────────────
        public SpectralXZombiePsycho()
        {
            Console.WriteLine("[SpectralXZombiePsycho] Created");
        }
          ...
          SpectralXScene scene, SpectralXMeshLibrary lib)
        {
         ...
         0f,
          0f
            ...
            Console.WriteLine($"[SpectralXZombiePsycho] Mesh initialized at ({spawnX},{spawnY},{spawnZ})");
        }
        ...
        public void Tick(float delta)
        {
            _runningTime += delta;
            // Hit phase
            if (_phase == ZombiePhase.HitEffect)
            {
            ...
            if (_isOneShotAnimation)
            {
                ...
                if (_animationTimer >= totalDuration)
                {
                    ...
                    _animationTimer = 0f;
                    SetAnimation(ZombieAnimationState.Idle);
                }
            }
        }
        ...
        {
            if (_phase == ZombiePhase.Dead) return;

            EnemyHitPoints = Math.Max(EnemyHitPoints - amount, 0);
            Console.WriteLine($"[ZombiePsycho] TakeDamage:{amount} HP:{EnemyHitPoints}/{EnemyMaxHP}");
        ...
        public void EnemyAttack(ISpectralCharacter target)
        {
            ...
            SetAnimation(ZombieAnimationState.Attack);
            _isOneShotAnimation = true;

            int frames = FrameCounts[ZombieAnimationState.Attack];
            float perFrame = AnimSpeeds[ZombieAnimationState.Attack];
        ...
        public void EnemyMove(ISpectralCharacter target)
        {
            ...
            if (effectiveTarget != null && effectiveTarget.CharIsAlive)
            {
                ...
                if (distSq > 0f)
                {
                    ...
                    float speed = EnemyCelerity * 0.03f; // slower, shambling
                    float vx = dx / dist;
                    float vy = dy / dist;
                    ...
                    if (MathF.Abs(vx) > MathF.Abs(vy))
                    {
                        if (vx > 0)
                        {
                            SetAnimation(ZombieAnimationState.WalkLeft);  // keep as is
                            _facingRight = true;   // was false — flip
                        }
                        else
                        {
                            SetAnimation(ZombieAnimationState.WalkRight);  // keep as is
                            _facingRight = false;  // was true — flip
                        }
                    }
                    else
                    {
                        // Zombies only have left/right/idle in this sheet set,
                        // so vertical movement keeps current horizontal anim.
                    }
                }
            ...
            float patrolSpeed = EnemyCelerity * 0.03f;

            if (_movingRight)
            {
                WorldX += patrolSpeed;
                SetAnimation(ZombieAnimationState.WalkRight);
            ...
            {
                WorldX -= patrolSpeed;
                SetAnimation(ZombieAnimationState.WalkLeft);
        ...
        // ── Animation ─────────────────────────────────────────────
        private void SetAnimation(ZombieAnimationState newState)
        {
        ...
        private void ApplyAnimationToMesh()
        {
            ...
            Console.WriteLine($"[ZombiePsycho] Animation → {_currentAnimation}");
        }
        ...
        // TODO: Zombie special attack when animations exist
        // TODO: SplatterPuddles when FX system is ported
        // TODO: Grayscale / rot shader for death state
    }
}
