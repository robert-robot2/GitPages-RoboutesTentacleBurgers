...
vec2 shadowFactorWithPenumbra(int lightIndex, vec3 normal, vec3 lightDir) {
    ...
    // ?? Per-pixel random rotation — world position hash
    // breaks staircase by making every pixel sample uniquely
    float randAngle = fract(sin(
        dot(vWorldPos.xy, vec2(127.1, 311.7))
    ) * 43758.5453) * 6.28318;
    float cosA = cos(randAngle);
    float sinA = sin(randAngle);

    // ?? Rotated Poisson Disk — 16 optimized samples
    vec2 poissonDisk[16];
    ...
    float radius = uShadowSoftnessBias * uShadowKernelSize;

    for (int i = 0; i < 16; i++) {
        vec2 s = poissonDisk[i];
        // Rotate sample by per-pixel random angle — breaks staircase pattern
        vec2 rotated = vec2(
            s.x * cosA - s.y * sinA,
            s.x * sinA + s.y * cosA
        ) * radius;

        float sampleDepth = sampleShadowMap(lightIndex, proj.xy + rotated);
    ...
    // Penumbra edge — samples split between lit/shadow = boundary
    float penumbraEdge = 1.0 - (penumbraAccum / 16.0) * 2.0;
